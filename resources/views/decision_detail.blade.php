    ...
    <div class="animate-fade">
                ...
                <div class="glass" style="padding: 1.5rem; border-color: rgba(57, 255, 20, 0.2);">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
                        <h3 style="color: var(--accent-toxic); font-size: 0.8rem; text-transform: uppercase;">Cognitive
                            Impact</h3>
                        <span class="badge"
                            style="border-color: var(--accent-cyber); color: var(--accent-cyber);">RESONANCE:
                            {{ $resonance['status'] }}</span>
                    </div>

                    <div style="margin-bottom: 1.5rem;">
                        <div
                            style="display: flex; justify-content: space-between; font-size: 0.75rem; margin-bottom: 0.4rem;">
                        ...
                        <div style="height: 4px; background: rgba(255,255,255,0.05); border-radius: 2px; overflow: hidden; margin-bottom: 1rem;">
                            <div
                        ...
                        <form action="{{ route('decisions.energy', $decision) }}" method="POST" style="display: flex; gap: 0.5rem; align-items: center;">
                            @csrf
                            <select name="energy_delta" style="background: rgba(0,0,0,0.5); border: 1px solid var(--glass-border); color: #fff; font-size: 0.7rem; padding: 0.3rem;">
                            ...
                            <button type="submit" class="btn" style="padding: 0.3rem 0.6rem; font-size: 0.6rem;">Log Energy</button>
                        </form>
                    </div>
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
            ...
            <div class="glass" style="padding: 1rem; border-color: var(--accent-crimson);">
                <h4 style="font-size: 0.7rem; color: var(--accent-crimson); text-transform: uppercase; margin-bottom: 1rem; margin-top: 0;">Record Outcome</h4>
                <form action="{{ route('decisions.outcomes', $decision) }}" method="POST" style="display: flex; gap: 1rem; align-items: flex-end;">
                    @csrf
                    <div>
                        <label style="display: block; font-size: 0.6rem; color: var(--text-muted); margin-bottom: 0.3rem;">IMPACT TYPE</label>
                        <select name="impact_type" style="background: rgba(0,0,0,0.5); border: 1px solid var(--glass-border); color: #fff; font-size: 0.7rem; padding: 0.4rem;">
                    ...
                    <div>
                        <label style="display: block; font-size: 0.6rem; color: var(--text-muted); margin-bottom: 0.3rem;">SEVERITY (0-5)</label>
                        <input type="number" name="severity" value="0" min="0" max="5" style="width: 50px; background: rgba(0,0,0,0.5); border: 1px solid var(--glass-border); color: #fff; font-size: 0.7rem; padding: 0.4rem;">
                    </div>
                    <div style="flex-grow: 1;">
                        <label style="display: block; font-size: 0.6rem; color: var(--text-muted); margin-bottom: 0.3rem;">MEASURABLE DELTA</label>
                        <input type="text" name="measurable_delta" required style="width: 100%; background: rgba(0,0,0,0.5); border: 1px solid var(--glass-border); color: #fff; font-size: 0.7rem; padding: 0.4rem;" placeholder="e.g., 2h downtime">
                    </div>
